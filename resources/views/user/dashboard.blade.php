    ...
    <section id="produk" class="container mx-auto px-4 sm:px-6 py-12">
        ...
        @if(request('search'))
            <p class="mb-6 text-gray-600 dark-mode-text">
                Hasil pencarian untuk: <span class="font-semibold text-gray-800 dark-mode-text-bold">"{{ request('search') }}"</span>
            </p>
        @endif
    ...
    @if ($produk->lastPage() > 1)
        <div class="mt-10 pb-12 flex justify-center">
            <nav class="pagination-nav flex flex-wrap items-center justify-center gap-2 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-xl shadow-lg">
                ...
                @if ($start > 1)
                    <a href="{{ $produk->url(1) }}" class="pagination-link px-3 py-2 text-blue-900 hover:bg-blue-50 rounded-lg transition">1</a>
                    @if ($start > 2)
                ...
                @for ($i = $start; $i <= $end; $i++)
                    @if ($i == $current)
                        <span class="pagination-active px-3 py-2 bg-blue-900 text-white rounded-lg font-semibold shadow-md">{{ $i }}</span>
                    @else
                        <a href="{{ $produk->url($i) }}" class="pagination-link px-3 py-2 text-blue-900 hover:bg-blue-50 rounded-lg transition">{{ $i }}</a>
                    @endif
                @endfor
                ...
                @if ($end < $last)
                    @if ($end < $last - 1)
                        <span class="text-gray-400 dark-mode-text">...</span>
                    @endif
                    <a href="{{ $produk->url($last) }}" class="pagination-link px-3 py-2 text-blue-900 hover:bg-blue-50 rounded-lg transition">{{ $last }}</a>
                @endif

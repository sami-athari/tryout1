...
@endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-16">
            <div class="bg-white rounded-2xl shadow-lg p-8 text-center border-t-4 border-blue-600">
                <h3 class="text-4xl font-bold text-gray-900 mb-2">{{ $totalProduk ?? '120+' }}</h3>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-8 text-center border-t-4 border-green-500">
                <h3 class="text-4xl font-bold text-gray-900 mb-2">{{ $userCount ?? '500+' }}</h3>
                <p class="text-gray-600">Active Users</p>
            </div>
            <div class="bg-white rounded-2xl shadow-lg p-8 text-center border-t-4 border-yellow-500">
                <h3 class="text-4xl font-bold text-gray-900 mb-2">{{ number_format($totalSold) }}</h3>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
            ...
            <p class="text-lg text-gray-700 leading-relaxed">{{ $about->mission ?? 'No mission yet.' }}</p>
        </div>

        <!-- Why -->
        <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
            <h2 class="text-3xl font-bold text-gray-900 mb-4">✨ Why Seilmu?</h2>
            <p class="text-lg text-gray-700 leading-relaxed">{{ $about->why ?? 'No reason yet.' }}</p>
        </div>
            ...
            @if(request('search'))
                <div class="mb-6">
                    <div class="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-full">
                        ...
                        <span class="font-medium">Results for "{{ request('search') }}"</span>
                    </div>
                </div>
            @endif
            ...
            <div class="mb-16">
                <nav class="flex justify-center">
                    <div class="flex items-center gap-2 bg-white rounded-full shadow-lg px-4 py-2">
                        ...
                        @else
                            <a href="{{ $produk->previousPageUrl() }}" class="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-full transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                </svg>
                            </a>
                        ...
                        @if ($start > 1)
                            <a href="{{ $produk->url(1) }}" class="px-4 py-2 text-gray-700 hover:bg-blue-50 rounded-full transition font-medium">1</a>
                            @if ($start > 2)
                        ...
                        @for ($i = $start; $i <= $end; $i++)
                            @if ($i == $current)
                                <span class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full font-semibold shadow-md">{{ $i }}</span>
                            @else
                                <a href="{{ $produk->url($i) }}" class="px-4 py-2 text-gray-700 hover:bg-blue-50 rounded-full transition font-medium">{{ $i }}</a>
                            @endif
                        @endfor
                        ...
                        @if ($end < $last)
                            @if ($end < $last - 1)
                                <span class="px-2 text-gray-400">...</span>
                            @endif
                            <a href="{{ $produk->url($last) }}" class="px-4 py-2 text-gray-700 hover:bg-blue-50 rounded-full transition font-medium">{{ $last }}</a>
                        @endif

                        @if ($produk->hasMorePages())
                            <a href="{{ $produk->nextPageUrl() }}" class="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-full transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

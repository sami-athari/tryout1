        ...
        <nav class="bg-white border-b sticky top-0 z-50">
            <div class="container mx-auto px-6 py-4">
                <div class="flex justify-between items-center">
                    <a href="{{ route('admin.dashboard') }}" class="text-2xl font-bold text-gray-900">Seilmu Admin</a>

                    <!-- Desktop Menu -->
                    <div class="hidden lg:flex items-center gap-6">
                        <a href="{{ route('admin.dashboard') }}" class="text-gray-700 hover:text-gray-900">Home</a>
                        <a href="{{ route('admin.about.index') }}" class="text-gray-700 hover:text-gray-900">About</a>
                        <a href="{{ route('admin.produk.index') }}" class="text-gray-700 hover:text-gray-900">Products</a>
                        <a href="{{ route('admin.kategori.index') }}" class="text-gray-700 hover:text-gray-900">Categories</a>
                        <a href="{{ route('admin.users.index') }}" class="text-gray-700 hover:text-gray-900">Users</a>
                        <a href="{{ route('admin.transactions.index') }}" class="text-gray-700 hover:text-gray-900">Orders</a>
                        ...
                        <div class="relative group">
                            <a href="{{ route('chat.index') }}" class="text-gray-700 hover:text-gray-900 flex items-center gap-1">
                                Chat
                                @if ($unreadCount > 0)
                                    <span class="bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{{ $unreadCount }}</span>
                                @endif
                            </a>
                                ...
                                @else
                                    <ul class="max-h-60 overflow-y-auto">
                                        @foreach ($unreadNotifications as $notif)
                                            <li>
                                                <a href="{{ route('chat.index', $notif->sender_id) }}" class="flex items-center p-3 hover:bg-gray-50">
                ...
                <div id="mobileMenu" class="hidden lg:hidden mt-4 space-y-4">
                    <a href="{{ route('admin.dashboard') }}" class="block py-2">Home</a>
                    <a href="{{ route('admin.about.index') }}" class="block py-2">About</a>
                    <a href="{{ route('admin.produk.index') }}" class="block py-2">Products</a>
                    <a href="{{ route('admin.kategori.index') }}" class="block py-2">Categories</a>
                    <a href="{{ route('admin.users.index') }}" class="block py-2">Users</a>
                    <a href="{{ route('admin.transactions.index') }}" class="block py-2">Orders</a>
                    <a href="{{ route('chat.index') }}" class="block py-2">Chat @if($unreadCount > 0)<span class="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{{ $unreadCount }}</span>@endif</a>
